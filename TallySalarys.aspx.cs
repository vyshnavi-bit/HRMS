...
public partial class TallySalarys : System.Web.UI.Page
{
    SqlCommand cmd;
    string userid = "";
    DBManager vdm;
    ...
    protected void btn_Generate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            foreach (DataRow dreport in Report.Select("DESIGNATION='" + designation + "'"))
            {
                ...
                string grearn = dreport["GROSS Earnings"].ToString();
                if (grearn != "")
                {
                    tgross = Convert.ToDouble(grearn);
                }
                else
                {
                    tgross = 0;
                }
                narration = "Being The SVDS " + ddlbranches.SelectedItem.Text +ddlemptype.SelectedItem.Text+ " salaries For the Month Of " + namonth + ", Total Gross=" + tgross + "/-,NetPay=" + tnetpay + "/- ";
                ...
                DataRow nr_ot = dtnew.NewRow();
                nr_ot["Invoice"] = invoice;
                nr_ot["JV Date"] = talydate;
                string othdeduction = "4";
                string otherdeduction_ledger = "";
                foreach (DataRow dr in dtledger.Select("sno='" + othdeduction + "'"))
                {
                    otherdeduction_ledger = dr["ledgername"].ToString();
                    otherdeduction_ledger = "other deduction";
                }
                nr_ot["Ledger Name"] = otherdeduction_ledger;
                nr_ot["Narration"] = narration;
                double otherded = 0;
                double.TryParse(dreport["OTHER DEDUCTION"].ToString(), out otherded);
                if (otherded == 0.0)
                {
                }
                else
                {
                    double otamt = 0;
                    double.TryParse(dreport["OTHER DEDUCTION"].ToString(), out otamt);
                    otamt = Math.Round(otamt, 2);
                    string otamount = "-" + otamt + "";
                    nr_ot["Amount"] = otamount;
                    dtnew.Rows.Add(nr_ot);
                }
