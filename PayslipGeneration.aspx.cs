...
public partial class PayslipGeneration : System.Web.UI.Page
{
    ...
    protected void btn_generate_click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (empid == "")
            {
                Response.Write("<script>alert('Please Enter Employeename');</script>");
                return;
            }
            ...
            for (i = 1; i <= noofmnts; i++)
            {
                ...
                foreach (DataRow dr in routes.Rows)
                {
                    ...
                    double NrOfDays = 0;
                    if (ddlmonth.SelectedItem.Text == "Last 3 Months" || ddlmonth.SelectedItem.Text == "Last 6 Months")
                    {
        ...
        try
        {
            ...
            for (i = 1; i <= noofmnts; i++)
            {
                if (dtroutes.Rows.Count > 0)
                {
                    ...
                    else if (ddlmonth.SelectedItem.Value == "2")
                    {
                        DateTime dtfrom = fromdate.AddMonths(-2);
                        string frmdate = dtfrom.ToString("MM/dd/yyyy");
                        string[] str = frmdate.Split('/');
                        int lastmonth = Convert.ToInt32(dtroutes.Rows[0]["fromdate"].ToString());
                        int years = Convert.ToInt32(str[2]);
    ...
    protected void PaySlipPDFGeneration()
    {
        ...
        if (strdate != "")
        {
            ...
            newstrdate = dtjoin.ToString("dd/MMM/yyyy");
        }
        ...
        //SendMail(msgTo, msgSubject, PaySlipPath);
