...
public partial class casualsupply : System.Web.UI.Page
{
    SqlCommand cmd;
    string userid = "";
    DBManager vdm;
        ...
        try
        {
            ...
            if (mainbranch == "42")
            {
                if (ddlbranch.SelectedItem.Text != "42" || ddlbranch.SelectedItem.Text != "43")
                {
                    ...
                    cmd = new SqlCommand("SELECT employebankdetails.ifsccode, employedetails.esieligible,employedetails.gender, employedetails.pfeligible, employedetails.salarymode, employedetails.empid,employedetails.employee_num, employedetails.fullname, designation.designation, salaryappraisals.salaryperyear, employebankdetails.accountno,salaryappraisals.gross, supply_attendance.month, supply_attendance.year FROM  employedetails INNER JOIN designation ON employedetails.designationid = designation.designationid INNER JOIN supply_attendance ON employedetails.empid = supply_attendance.empid INNER JOIN salaryappraisals ON employedetails.empid = salaryappraisals.empid LEFT OUTER JOIN employebankdetails ON employedetails.empid = employebankdetails.employeid WHERE (employedetails.branchid = @branchid) AND (supply_attendance.month = @month) AND (supply_attendance.year = @year) AND (employedetails.salarymode = '1') AND (employedetails.employee_type = 'Casuals' OR employedetails.employee_type = 'Casual worker') AND (employedetails.status = 'No') AND (salaryappraisals.endingdate IS NULL) AND (salaryappraisals.startingdate <= @d1) OR (employedetails.branchid = @branchid) AND (supply_attendance.month = @month) AND (supply_attendance.year = @year) AND (employedetails.salarymode = '1') AND (employedetails.employee_type = 'Casuals' OR employedetails.employee_type = 'Casual worker') AND (employedetails.status = 'No') AND (salaryappraisals.endingdate > @d1) AND (salaryappraisals.startingdate <= @d1)ORDER BY employedetails.gender,employedetails.employee_num");
                    //paystrure
                    //cmd = new SqlCommand("SELECT employebankdetails.ifsccode, employedetails.gender, employedetails.pfeligible, employedetails.salarymode, employedetails.empid, employedetails.employee_num, employedetails.fullname, designation.designation, pay_structure.salaryperyear, employebankdetails.accountno, pay_structure.gross, supply_attendance.month, supply_attendance.year FROM employedetails INNER JOIN designation ON employedetails.designationid = designation.designationid INNER JOIN pay_structure ON employedetails.empid = pay_structure.empid INNER JOIN supply_attendance ON employedetails.empid = supply_attendance.empid LEFT OUTER JOIN employebankdetails ON employedetails.empid = employebankdetails.employeid WHERE (employedetails.branchid = @branchid) AND (supply_attendance.month = @month) AND (supply_attendance.year = @year) AND (employedetails.salarymode = '1') AND ((employedetails.employee_type = 'Casuals') OR (employedetails.employee_type ='Casual worker')) AND employedetails.status='No' ORDER BY employedetails.gender");
                    cmd.Parameters.Add("@branchid", branchid);
                    cmd.Parameters.Add("@month", mymonth);
                    cmd.Parameters.Add("@year", year);
                    cmd.Parameters.Add("@d1", date);
                    DataTable dtsalary = vdm.SelectQuery(cmd).Tables[0];
                    cmd = new SqlCommand("SELECT supply_attendance.empid, supply_attendance.clorwo,supply_attendance.doe, supply_attendance.month, supply_attendance.year, supply_attendance.extradays, employedetails.employee_num, branchmaster.company_code, branchmaster.fromdate, branchmaster.todate, supply_attendance.lop, branchmaster.branchid, supply_attendance.sno, supply_attendance.numberofworkingdays FROM  supply_attendance INNER JOIN employedetails ON supply_attendance.empid = employedetails.empid INNER JOIN  branchmaster ON branchmaster.branchid = employedetails.branchid WHERE (supply_attendance.month = @month) AND (supply_attendance.year = @year) AND (employedetails.branchid = @branchid)");
                    cmd.Parameters.Add("@month", mymonth);
                    cmd.Parameters.Add("@year", year);
                    cmd.Parameters.Add("@branchid", branchid);
                        ...
                        foreach (DataRow dr in dtsalary.Rows)
                        {
                                ...
                                if (pf == "Yes")
                                {
                                    ...
                                    if (dtattendence.Rows.Count > 0)
                                    {
                                        foreach (DataRow dra in dtattendence.Select("employee_num='" + dr["employee_num"].ToString() + "'"))
                                        {
                                            ...
                                            if (mainbranch == "42")
                                            {
                                                if (ddlbranch.SelectedItem.Value == "1043" || ddlbranch.SelectedItem.Value == "1049" || ddlbranch.SelectedItem.Value == "1048" || ddlbranch.SelectedItem.Value == "1047")
                                                {
                                                    if (esieligible == "Yes")
                                                    {

                                                        esi = (paydaysesi * 1.75) / 100;
                            ...
                            else
                            {
                                if (pf != "Yes")
                                {
                                    ...
                                    //dtnewrow["Designation"] = dr["designation"].ToString();
                                    double peryanam = Convert.ToDouble(dr["salaryperyear"].ToString());
            ...
            else
            {
                ...
                else if (ddlbranch.SelectedItem.Value == "39")
                {
                    ...
                    //cmd = new SqlCommand("SELECT supply_attendance.sno, supply_attendance.empid, supply_attendance.doe, supply_attendance.month, supply_attendance.year, supply_attendance.otdays, employedetails.employee_num, branchmaster.fromdate, branchmaster.todate, monthly_workingdays.numberofworkingdays, supply_attendance.lop,  branchmaster.branchid FROM  supply_attendance INNER JOIN employedetails ON supply_attendance.empid = employedetails.empid INNER JOIN branchmaster ON employedetails.branchid = branchmaster.branchid INNER JOIN monthly_workingdays ON branchmaster.branchid = monthly_workingdays.branchid WHERE (supply_attendance.month = @month) AND (supply_attendance.year = @year) AND (employedetails.branchid=@branchid)");
                    cmd.Parameters.Add("@month", mymonth);
                    cmd.Parameters.Add("@year", year);
                    cmd.Parameters.Add("@branchid", branchid);
                    DataTable dtattendence = vdm.SelectQuery(cmd).Tables[0];
